...
// renderSpreadsheetBody fetches job data and renders the spreadsheet_body partial.
func (h *Handler) renderSpreadsheetBody(w http.ResponseWriter, r *http.Request, jobID string, orgID uuid.NullUUID) {
	ctx := r.Context()
	logger := middleware.LoggerFromContext(ctx)
...
func (h *Handler) CreateLineItem(w http.ResponseWriter, r *http.Request) {
	...
	// Spreadsheet context: return partial HTML instead of redirect
	if jobID := r.FormValue("job_id"); jobID != "" && r.Header.Get("HX-Request") == "true" {
		h.renderSpreadsheetBody(w, r, jobID, GetOrgID(ctx))
		return
	}
