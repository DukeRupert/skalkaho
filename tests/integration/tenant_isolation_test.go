...
func TestTenantIsolation_OrgIDColumnsExist(t *testing.T) {
	...
	tables := []string{"jobs", "categories", "line_items", "settings", "clients", "estimates"}

	for _, table := range tables {
		t.Run(table+"_has_org_id", func(t *testing.T) {
			// Query table info to check for org_id column
			var count int
			err := db.QueryRow(`
				SELECT COUNT(*) FROM pragma_table_info(?) WHERE name = 'org_id'
			`, table).Scan(&count)
...
func TestTenantIsolation_Jobs(t *testing.T) {
	...
	_, err := db.ExecContext(ctx, `
		...
		VALUES
			('job-a1', 'org-a', 'Org A - Kitchen Remodel', 15.0, 'stacking', 'draft', datetime('now')),
			('job-a2', 'org-a', 'Org A - Bathroom Addition', 20.0, 'stacking', 'draft', datetime('now')),
			('job-b1', 'org-b', 'Org B - Deck Construction', 10.0, 'stacking', 'draft', datetime('now'))
	`)
	...
	t.Run("ListJobs_filtered_by_org_id", func(t *testing.T) {
		...
		rows, err := db.QueryContext(ctx, `SELECT id, name FROM jobs WHERE org_id = ?`, "org-a")
		require.NoError(t, err)
		defer rows.Close()
	...
	t.Run("CountJobs_scoped_by_org", func(t *testing.T) {
		...
		err := db.QueryRowContext(ctx, `SELECT COUNT(*) FROM jobs WHERE org_id = ?`, "org-a").Scan(&countA)
		require.NoError(t, err)
		err = db.QueryRowContext(ctx, `SELECT COUNT(*) FROM jobs WHERE org_id = ?`, "org-b").Scan(&countB)
		require.NoError(t, err)
	...
	t.Run("UpdateJob_respects_org_boundary", func(t *testing.T) {
		...
		result, err := db.ExecContext(ctx, `
			UPDATE jobs SET name = 'HACKED' WHERE id = ? AND org_id = ?
		`, "job-b1", "org-a")
		require.NoError(t, err)
		...
		var name string
		err = db.QueryRowContext(ctx, `SELECT name FROM jobs WHERE id = ?`, "job-b1").Scan(&name)
		require.NoError(t, err)
	...
	t.Run("DeleteJob_respects_org_boundary", func(t *testing.T) {
		...
		var count int64
		err = db.QueryRowContext(ctx, `SELECT COUNT(*) FROM jobs WHERE id = ?`, "job-b1").Scan(&count)
		require.NoError(t, err)
...
func TestTenantIsolation_Categories(t *testing.T) {
	...
	_, err := db.ExecContext(ctx, `
		INSERT INTO jobs (id, org_id, name, surcharge_percent, surcharge_mode, status, created_at)
		VALUES
			('job-a', 'org-a', 'Org A Job', 15.0, 'stacking', 'draft', datetime('now')),
			('job-b', 'org-b', 'Org B Job', 10.0, 'stacking', 'draft', datetime('now'))
	`)
	require.NoError(t, err)
	...
	_, err = db.ExecContext(ctx, `
		...
		VALUES
			('cat-a1', 'org-a', 'job-a', 'Org A - Foundation', 0),
			('cat-a2', 'org-a', 'job-a', 'Org A - Framing', 1),
			('cat-b1', 'org-b', 'job-b', 'Org B - Plumbing', 0)
	`)
	...
	t.Run("ListCategories_filtered_by_org", func(t *testing.T) {
		var count int64
		err := db.QueryRowContext(ctx, `SELECT COUNT(*) FROM categories WHERE org_id = ?`, "org-a").Scan(&count)
		require.NoError(t, err)
...
func TestTenantIsolation_LineItems(t *testing.T) {
	...
	_, err = db.ExecContext(ctx, `
		INSERT INTO categories (id, org_id, job_id, name, sort_order)
		VALUES
			('cat-a', 'org-a', 'job-a', 'Org A Category', 0),
			('cat-b', 'org-b', 'job-b', 'Org B Category', 0)
	`)
	require.NoError(t, err)
	...
	_, err = db.ExecContext(ctx, `
		...
		VALUES
			('item-a1', 'org-a', 'cat-a', 'material', 'Org A - 2x4 Lumber', 100, 'ea', 5.50, 0),
			('item-b1', 'org-b', 'cat-b', 'material', 'Org B - Copper Pipe', 50, 'ea', 12.00, 0)
	`)
	...
	t.Run("GetLineItem_respects_org_boundary", func(t *testing.T) {
		var id string
		err := db.QueryRowContext(ctx, `
			SELECT id FROM line_items WHERE id = ? AND org_id = ?
		`, "item-b1", "org-a").Scan(&id)
	...
	t.Run("LineItem_inherits_org_through_hierarchy", func(t *testing.T) {
		...
		var itemOrg, catOrg, jobOrg string
		err := db.QueryRowContext(ctx, `
...
func TestTenantIsolation_Settings(t *testing.T) {
	...
	// Create settings for two orgs
	_, err := db.ExecContext(ctx, `
		INSERT INTO settings (id, org_id, default_surcharge_mode, default_surcharge_percent)
		VALUES
			('settings-a', 'org-a', 'stacking', 15.0),
			('settings-b', 'org-b', 'override', 20.0)
	`)
	...
	t.Run("Settings_isolated_between_orgs", func(t *testing.T) {
		...
		var countA, countB int64
		db.QueryRowContext(ctx, `SELECT COUNT(*) FROM settings WHERE org_id = ?`, "org-a").Scan(&countA)
		db.QueryRowContext(ctx, `SELECT COUNT(*) FROM settings WHERE org_id = ?`, "org-b").Scan(&countB)
...
func TestTenantIsolation_Clients(t *testing.T) {
	...
	_, err := db.ExecContext(ctx, `
		...
		VALUES
			('client-a1', 'org-a', 'John Doe', 'john@example.com', datetime('now')),
			('client-b1', 'org-b', 'Jane Smith', 'jane@example.com', datetime('now'))
	`)
...
func TestTenantIsolation_Estimates(t *testing.T) {
	...
	_, err = db.ExecContext(ctx, `
		...
		VALUES
			('est-a1', 'org-a', 'job-a', 1, 'draft', 5000.00, datetime('now')),
			('est-b1', 'org-b', 'job-b', 1, 'draft', 8000.00, datetime('now'))
	`)
	...
	t.Run("GetEstimate_respects_org_boundary", func(t *testing.T) {
		var id string
		err := db.QueryRowContext(ctx, `
			SELECT id FROM estimates WHERE id = ? AND org_id = ?
		`, "est-b1", "org-a").Scan(&id)
...
func TestTenantIsolation_CrossOrgPrevention(t *testing.T) {
	...
	t.Run("Cannot_attach_category_to_other_orgs_job", func(t *testing.T) {
		...
		_, err := db.ExecContext(ctx, `
			INSERT INTO categories (id, org_id, job_id, name, sort_order)
			VALUES ('malicious-cat', 'org-a', 'job-b', 'Malicious Category', 0)
		`)
	...
	t.Run("Cannot_attach_line_item_to_other_orgs_category", func(t *testing.T) {
		...
		_, err := db.ExecContext(ctx, `
			INSERT INTO line_items (id, org_id, category_id, type, name, quantity, unit, unit_price, sort_order)
			VALUES ('malicious-item', 'org-a', 'cat-b', 'material', 'Malicious Item', 1, 'ea', 100, 0)
		`)
		// This should fail due to org_id mismatch constraint
	...
	t.Run("Cannot_link_job_to_other_orgs_client", func(t *testing.T) {
		...
		_, err := db.ExecContext(ctx, `
			...
			VALUES ('client-b', 'org-b', 'Org B Client', datetime('now'))
		`)
		require.NoError(t, err)

		// Try to link org-a's job to org-b's client
		_, err = db.ExecContext(ctx, `
			UPDATE jobs SET client_id = 'client-b' WHERE id = 'job-a' AND org_id = 'org-a'
		`)
